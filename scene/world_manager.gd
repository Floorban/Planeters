...
func consume_selected_character_for_sacrifice(character: Character) -> bool:
	...
	if not cultists.has(character):
		return false
	cultists.erase(character)
	current_members = max(0, current_members - 1)
	if cur_character == character:
		cur_character = null
	if last_released_character == character:
		last_released_character = null
	character.start_being_killed()
	GameManager.stats_manager.spend_stat(GameManager.sim_manager.member_stat, 1)
	return true
